...
import kr.hyfata.rest.api.auth.repository.LoginHistoryRepository;
...
/**
 * SAS OAuth2AuthorizationService 세션 브리징 데코레이터
 * <p>
 * SAS의 토큰 저장/삭제 시점에 기존 user_sessions 세션 관리를 연동한다:
 * - save(): refresh token이 새로 포함된 저장(최초 발급 또는 로테이션) 시 세션 미러링.
 *   최초 발급 시 LoginHistory도 기록한다.
 * - remove(): SAS authorization 제거(로그아웃, refresh token revoke, reuse 감지) 시
 *   연결된 세션 무효화 + Access Token JTI 블랙리스트 등록.
 * <p>
